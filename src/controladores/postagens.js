...
const novaPostagem = async (req, res) => {
  ...
  if (!fotos || fotos.length === 0) {
    return res.status(400).json({ message: "É necessário ao mens uma foto" });
  }
  ...
  try {
    ...
    if (!postagem) {
      return res
        .status(400)
        .json({ message: "Não foi possível cadastrar a postagem" });
    }
    ...
    return res.status(201).json({ message: "Postagem concluída com sucesso" });
  } catch (error) {
...
const curtir = async (req, res) => {
  ...
  try {
    ...
    if (jaCurtiu) {
      return res
        .status(400)
        .json({ message: "O usuário já curtiu a postagem" });
    }
...
const comentar = async (req, res) => {
  ...
  if (!texto) {
    return res.status(400).json({ message: "É preciso inserir um comentário" });
  }
  ...
  try {
    ...
    return res.status(201).json({ message: "Comentário inserido com sucesso" });
  } catch (error) {
    return res.status(400).json(error.message);
  }
};
